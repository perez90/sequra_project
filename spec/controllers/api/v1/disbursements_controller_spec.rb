...
RSpec.describe Api::V1::DisbursementsController, type: :controller do
  describe 'GET #index' do
    before do
      ...
      Disbursement.create(amount: 200.20, merchant_id: merchant_1.id, year: Time.current.year,
                          week: Time.current.strftime('%U').to_i)
      Disbursement.create(amount: 300.20, merchant_id: merchant_2.id, year: Time.current.year,
                          week: Time.current.strftime('%U').to_i)
    end
